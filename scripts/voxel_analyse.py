...
def morton_encode(x: np.uint64, y: np.uint64, z: np.uint64) -> np.uint64:
    return (
        split_by_3_scalar(x) | (split_by_3_scalar(y) << 1) | (split_by_3_scalar(z) << 2)
    )
...
def build_voxel_pairs(points: np.ndarray, connectivity: np.ndarray, resolution: float):
    ...
    # Shift coordinates so everything is positive (Morton codes require unsigned ints)
    ...
    # A tet might span multiple voxels. We use its Axis-Aligned Bounding Box (AABB)

    # to find all intersected voxels. This first pass calculates exact array sizes.
...
def analyze_voxel_hashmap(mesh, resolution: float):
    ...
    console.print(
        f"\n[bold white]Max Tets/Voxel:[/bold white] [red]{tets_per_voxel.max()}[/red] | [bold white]Avg:[/bold white] [blue]{tets_per_voxel.mean():.1f}[/blue]"
    )


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("mesh", type=Path, help="Path to mesh to analyze")
