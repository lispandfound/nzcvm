...
class TestQueryMany:
    """query_many returns a float32 ndarray of shape (*x.shape, 6)."""

    def test_shape_and_dtype(self):
        x = np.array([0.1, 0.2], dtype=np.float32)
        z = np.zeros(2, dtype=np.float32)
        result = _make_model().query_many(x, z, z)
        assert result.shape == (2, 6) and result.dtype == np.float32

    def test_nd_input(self):
        x = np.full((3, 4), 0.1, dtype=np.float32)
        assert _make_model().query_many(x, x, x).shape == (3, 4, 6)

    def test_model_range_filters(self):
        x = np.array([0.1], dtype=np.float32)
        result = _make_model(vs=3500.0, priority=10).query_many(x, x, x, model_range=ModelRange.TOMOGRAPHY)
        assert abs(float(result[0, 2]) - 3500.0) < 1.0

    def test_blend_mode_over(self):
        x = np.array([0.1], dtype=np.float32)
        buf = np.zeros((1, 6), dtype=np.float32)
        assert _make_model().query_many(x, x, x, buffer=buf, blend_mode=BlendMode.Over).shape == (1, 6)
...
class TestDask:
    def test_query_many_via_apply_ufunc(self):
